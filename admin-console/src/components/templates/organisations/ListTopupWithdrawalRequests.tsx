...
interface ListTopupWithdrawalRequestProps {
  fetchTopupWithdrawalRequests: any;
  exportTopupWithdrawalRequests: any;
  defaultFilters: any;
  approveRequest: any;
  rejectRequest: any;
}

const ListTopupWithdrawalRequests: FC<ListTopupWithdrawalRequestProps> = ({
  fetchTopupWithdrawalRequests,
  exportTopupWithdrawalRequests,
  defaultFilters,
  approveRequest,
  rejectRequest,
}) => {
  ...
  const topupWithdrawalRequests = quotaPackagesRedux.getters.topupWithdrawalRequests(rootState);
  ...
  const columns: ColumnProps[] = useMemo(
    () => [
      { label: "Request ID", sortKey: "request_id" },
      {
        ...
        filters: topupWithdrawalRequests?.list?.filter_values?.organisation?.map((item) => ({ label: item?.org_reg_code, value: item?.org_reg_code })),
      },
      {
        label: "Organisation",
        align: "center",
        filterKey: "organisation_id",
        filters: topupWithdrawalRequests?.list?.filter_values?.organisation?.map((item) => ({ label: `${item?.name} (${item?.org_id})`, value: item?.id })),
      },
      {
        label: "Provider",
        align: "center",
        filterKey: "provider_id",
        filters: topupWithdrawalRequests?.list?.filter_values?.provider?.map((item) => ({ label: `${item?.provider_name} (${item?.provider_id})`, value: item?.id })),
      },
      {
        label: "Provider Location",
        align: "center",
        filterKey: "provider_location",
        filters: topupWithdrawalRequests?.list?.filter_values?.provider_location,
        defaultHidden: true,
      },
      { label: "Quota Package", align: "center" },
      { label: "Topup Type", align: "center", filterKey: "resource_topup_topup_type", filters: topupWithdrawalRequests?.list?.filter_values?.resource_topup_topup_type },
      { label: "Topup Value", align: "center" },
      { label: "Request Origin", align: "center", filterKey: "request_origin", filters: topupWithdrawalRequests?.list?.filter_values?.request_origin, defaultHidden: true },
      { label: "Status", align: "center", sortKey: "status", filterKey: "status", filters: topupWithdrawalRequests?.list?.filter_values?.status },
      { label: "Requested On", align: "center", sortKey: "created", defaultSort: "desc" },
      { label: "Withdrawn On", align: "center", sortKey: "withdrawal_date", defaultHidden: true },
    ],
    [topupWithdrawalRequests?.list],
  );
  ...
  const actions: ActionProps[] = [
    {
      label: () => "View Request",
      onClick: (item) => navigate(`${item?.id}/request-details`, { state: { quotaWithdrawRequestDetail: item } }),
    },
    {
    ...
    {
      label: () => "Audit Logs",
      onClick: (item) => navigate("/audit-logs/admin-trails", { state: { defaultFilters: { resource_id: [item?.id], resource_type: ["Resource Topup Withdrawal Request"] } } }),
    },
  ];

  const reload = useCallback(
    ({ limit, offset, search, order, orderBy, filters }) => fetchTopupWithdrawalRequests({ limit, offset, search, sort_by: orderBy, sort_asc: order === "asc", filters }),
    [fetchTopupWithdrawalRequests],
  );

  const dataList = useDataList({
    data: topupWithdrawalRequests?.list?.data || [],
    totalRecords: topupWithdrawalRequests?.totalRecords,
    columns,
    actions,
    exportFilename: "Topup Withdrawal Requests List",
    exportColumns,
    defaultFilters,
    rowCreator,
    exportCreator,
    searchFields,
    listExporter: exportTopupWithdrawalRequests,
    reload,
  });
...
export default ListTopupWithdrawalRequests;

...
interface ListPublicIpProps {
  fetchPublicIpWithdraw: any;
  exportPublicIpWithdraw: any;
  fetchWithdrawalOwnerDetail: any;
}

const ListPublicIpWithdrawRequests: FC<ListPublicIpProps> = ({ fetchPublicIpWithdraw, exportPublicIpWithdraw, fetchWithdrawalOwnerDetail }) => {
  const navigate = useNavigate();
  const rootState = useSelector((state: any) => state);
  const publicIps = networkRedux.getters.withdrawPublicIPs(rootState);
  ...
  const actions: ActionProps[] = [
    {
      label: () => "View Request",
      onClick: (item) => navigate(`${item?.id}/request-details`, { state: { publicIpWithdrawRequestDetail: item } }),
    },
    {
      label: () => "View Owner Details",
      onClick: (item) => fetchWithdrawalOwnerDetail(item),
    },
    {
      label: () => "Audit Logs",
      onClick: (item) => navigate("/audit-logs/admin-trails", { state: { defaultFilters: { resource_id: [item?.id], resource_type: ["PublicIPWithdrawalRequest"] } } }),
    },
  ];

  const reload = useCallback(
    ({ limit, offset, search, order, orderBy, filters }) => fetchPublicIpWithdraw({ limit, offset, search, sort_by: orderBy, sort_asc: order === "asc", filters }),
    [fetchPublicIpWithdraw],
  );
  ...
  const dataList = useDataList({
    ...
    columns,
    exportFilename: "Public IP Withdraw Requests List",
    exportColumns,
    searchFields,
    actions,
    rowCreator,
    exportCreator,
    listExporter: exportPublicIpWithdraw,
    reload,
  });
...
export default ListPublicIpWithdrawRequests;

...
interface ListComputeSGRulesProps {
  securityGroups: any;
...
const ListComputeSGRules: FC<ListComputeSGRulesProps> = ({ securityGroups, fetchSecurityGroupRules, exportSecurityGroupRules, handleDelete }) => {
  ...
  const dataList = useDataList({
    ...
    rowCreator,
    exportCreator,
    ...(handleDelete && securityGroups?.status.toLowerCase() !== "deleted"
      ? { deleteResourceButton: (checkedRows) => ({ text: "Delete Rules", onClick: () => handleDelete(checkedRows), disabled: !checkedRows?.length }) }
      : {}),
    checkbox: securityGroups?.status.toLowerCase() !== "deleted",
    rowCheckboxHidden: (item) => item?.managed_by?.toLowerCase() === "user" || item?.status?.toLowerCase() === "deleted",
...
export default ListComputeSGRules;

...
const Overview: FC<OverviewProps> = ({ securityGroup }) => {
      ...
      <Box sx={{ flexGrow: 1, bgColor: "background.paper", display: "flex", direction: { xs: "column", md: "row" } }}>
        ...
        <TabPanel value={tab} index={0} containerStyle={{ width: "100%" }}>
          <Stack spacing={1} width="100%">
            <Item label="Name" value={securityGroup?.security_group_name} />
            <Item label="Managed By" value={securityGroup?.managed_by} />
            <Item label="Security Group Type" value={securityGroup?.security_group_type} />
            <Item label="Status" value={securityGroup?.status} />
            <Item label="Created" value={securityGroup?.created ? <DateDisplay datetime={securityGroup?.created} /> : null} />
            <Item label="Updated" value={securityGroup?.updated ? <DateDisplay datetime={securityGroup?.updated} /> : null} />
            <Item label="Age" value={securityGroup?.created ? getAge(securityGroup?.created) : null} />
          </Stack>
        </TabPanel>
      </Box>
    </Box>

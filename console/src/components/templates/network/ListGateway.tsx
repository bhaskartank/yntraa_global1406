...
interface ListGatewayProps {
  list: any[];
  totalRecords: number;
  fetchList: (payload: any) => void;
}

const ListGateway: FC<ListGatewayProps> = ({ list, totalRecords, fetchList }) => {
      ...
      content: (
        <GridViewWrapper>
          <GridViewSection>
            ...
            <KeyValuePair label="Projects" value={<TagList tags={item?.organisation_project_user_user?.map((project) => project?.project?.name)} />} />
          </GridViewSection>

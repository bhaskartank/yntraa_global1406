...
class Namespace(BaseNamespace):
    ...
    def login_required(self, oauth_scopes, locations=('headers',)):
        #log.info("in login required ============>> %s", oauth_scopes)
        """
        ...
        * All of the above requirements are put into OpenAPI Specification with
        ...
        def decorator(func_or_class):
            ...
            # log.info(_oauth_scopes)

            oauth_protection_decorator = oauth2.require_oauth(*_oauth_scopes, locations=locations)
            # log.info(" in Namespace  oauth_protection_decorator")
            # log.info(oauth_protection_decorator)
            ...
            # log.info("oauth_protected_func ==================>> %s", oauth_protected_func)
            return self.doc(
                security={
    ...
    def _register_access_restriction_decorator(self, func, decorator_to_register): # pylint: disable=invalid-name
        """
        ...
        def decorator(func):
            ...
            def wrapper(self_, parameters_args, *args, **kwargs):
                ...
                return (
                    ...
                    HTTPStatus.OK,
                    {'X-Total-Count': total_count, 'Access-Control-Expose-Headers':'X-Total-Count'}
    ...
    def sort(self, parameters=None, locations=None):
        """
        field and order is required.
        Endpoint parameters registration decorator special for sorting.
        If ``parameters`` is not provided default SortingParameters will be used.
        Also, any custom Parameters can be used, but it needs to have ``field`` and ``order``
        fields.
        ...
        if not parameters:
            # Use default parameters if None specified
            from app.extensions.api.parameters import SortingParameters
            parameters = SortingParameters()

        if not all(
                mandatory in parameters.declared_fields
                for mandatory in ('field', 'order')
        ):
            raise AttributeError(
                '`field` and `order` fields must be in Parameter passed to `sort()`'
            )
        ...
        def decorator(func):
            ...
            @wraps(func)
            def wrapper(self_, parameters_args, *args, **kwargs):
                if 'field' in parameters_args:
                    args = ()

                queryset = func(self_, parameters_args, *args, **kwargs)

                field = parameters_args['field']
                order_by_param = parameters_args['order']

                if '.' in field:
                    table, field = field.split('.')
                    try:
                        queryset = queryset.join(table)
                    except InvalidRequestError:
                        error_msg = f"Provided nested table name '{table}' is invalid."
                        log.error(error_msg)

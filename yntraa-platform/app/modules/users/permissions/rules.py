...
class ActiveUserRoleRule(DenyAbortMixin, Rule):
    ...
    def check(self):
        ...
        self.DENY_ABORT_HTTP_CODE = HTTPStatus.FORBIDDEN
...
class TeamRoleRule(ActiveUserRoleRule):
    """
    Ensure that the current_user has an Team access to the given object.
    """

    def __init__(self, obj, **kwargs):
        super(TeamRoleRule, self).__init__(**kwargs)
        self._obj = obj

    def check(self):
        if not hasattr(self._obj, 'check_team'):
            return False
        return self._obj.check_team(current_user) is True


class ProjectRoleRule(ActiveUserRoleRule):
    """
    Ensure that the current_user has an project access to the given object.
    """

    def __init__(self, obj, **kwargs):
        super(ProjectRoleRule, self).__init__(**kwargs)
        self._obj = obj

    def check(self):

        if not hasattr(self._obj, 'check_project'):
            return False
        return self._obj.check_project(current_user) is True
